...
public class TeleSignRequestTest {
	...
	public void requestCreation() {
		TeleSignRequest tr;
		if(!timeouts)
			tr = new TeleSignRequest(HTTPS_REST_TELESIGN_COM, "/v1/phoneid/standard/15551234567", "GET", "customer_id", "secret_key");
		else
			tr = new TeleSignRequest(HTTPS_REST_TELESIGN_COM, "/v1/phoneid/standard/15551234567", "GET", "customer_id", "secret_key", connectTimeout, readTimeout);
		assertNotNull(tr);
	...
	public void shaMethodTest() throws IOException {
		...
		TeleSignRequest tr;
		if(!timeouts)
			tr = new TeleSignRequest(HTTPS_REST_TELESIGN_COM, "/v1/phoneid/standard/" + PHONE_NUMBER, "GET", CUSTOMER_ID, SECRET_KEY);
		else
			tr = new TeleSignRequest(HTTPS_REST_TELESIGN_COM, "/v1/phoneid/standard/" + PHONE_NUMBER, "GET", CUSTOMER_ID, SECRET_KEY, connectTimeout, readTimeout);
